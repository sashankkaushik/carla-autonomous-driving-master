...
class CarlaGame(object):
    ...
    def _on_loop(self):
        ...
        ######################### Agent control ###########################
        '''
        Pass all the required state representations 
        Using the policy agent returns action

        Must define a reward function so that we can give our agent feedback 
        about the actions taking, then it gets trained.
        '''
        if  self.agent:
            # control = self._control_using_agent( play_agent(self._main_image, self._mini_view_image1, 
                                                    #  self._mini_view_image2,self._lidar_measurement) )
            control = self._control_using_agent([1,0,0,0,0,0])
        
            '''
            Here we are updating the agent for the past action because, the current sensor 
            outputs are the results of previous action and the result of the current action is
            obtained in the next loop or we can write code to get the new outputs here
            ...
            '''
            # train(self._main_image, self._mini_view_image1, self._mini_view_image2, self._lidar_measurement)
...
def main():
    ...
    argparser.add_argument(
        ...
        type=bool,
        default=False,
        help='Specify True if you want agent to control or False for manual control')
